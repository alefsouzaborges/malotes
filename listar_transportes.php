...
include_once('classes/config-database.php');
		...
		$sqlTransportes = "SELECT * FROM transportes";
		$sqlTransportes = $pdo->prepare($sqlTransportes);
		$sqlTransportes->execute();
		...
		<div class="row justify_row justify-content-center" >
			<div class="box-form-listar-usuarios">
				...
				<table class="table table-sm table-striped table-hover table-light " style="color: #000; border-radius: 5px;">
				  ...
				  <tbody>
				  	<?php foreach($sqlTransportes->fetchAll() as $dados):  ?>
					  <tr>
				      <td align="center"><a href="alterar_transportes.php?id=<?php echo $dados['id']; ?>"><?php echo $dados['id']?></a></td>
				      <td><a href="alterar_transportes.php?id=<?php echo $dados['id']; ?>"><?php echo $dados['tipo_transporte']?></a></td>
				  	<?php endforeach; ?>
				  </tbody>
				</table>
