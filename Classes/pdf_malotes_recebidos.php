...
try {
	...
	WHERE  remetente = $unidade_logada  AND movimentoc.status = 'Recebido' or destinatario = $unidade_logada
	ORDER BY lacre DESC";
	$sql = $pdo->prepare($sql);
	$sql->execute();
	...
	$pdf->Cell(190,10,utf8_decode('Relatório de malotes recebidos'),0,0,"C");
