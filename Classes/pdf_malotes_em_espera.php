...
try {
	...
	WHERE  remetente = $unidade_logada  AND movimentoc.status = 'Aguardando Confirmação' or destinatario = $unidade_logada
	ORDER BY lacre DESC";
	$sql = $pdo->prepare($sql);
	$sql->execute();
	...
	$pdf->Cell(190,10,utf8_decode('Relatório de malotes em espera'),0,0,"C");
	$pdf->Ln(15);
	...
	foreach ($sql as $dados) {
		...
		$pdf->Cell(38,7,utf8_decode($dados["status"]),1,0,"C");
